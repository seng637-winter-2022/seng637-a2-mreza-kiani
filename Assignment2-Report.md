...
| Range         |          shift          |                                          Shifts the range by the specified amount.                                          |                                                                                                                 We used Robustness testing and Boundary value analysis for this method.<br>For Robustness testing, the method does not accept a null value so we used Range = null.<br>For boundary value, we used Range = (-1,0), delta values of 2,-1, 0 and both True/False for allowZeroCrossing                                                                                                                  |
| Range         |       isNaNRange        |                    Returns true if both the lower and upper bounds are Double.NaN, and false otherwise.                     |                                                                                                                                                                                     Since this was a relatively simple method, we only used Equivalance testing for it<br>We used Range values of (NaN,1), (1,NaN) and (NaN,NaN)                                                                                                                                                                                      |
| Range         |     expandToInclude     |                Returns a range that includes all the values in the specified range AND the specified value.                 |                                                                                                                                                                         For this method, we utilized equivalence testing and robust testing<br>The values for Range were null, (0,1) and (0,10). Values for expansion value were 10,-1 and 5                                                                                                                                                                          |
